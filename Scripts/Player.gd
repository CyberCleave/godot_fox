							...
							if Input.is_action_just_pressed("player_jump"):
									if jump_count > 0 && jump_count < doublejump_limit:
										if vel.y < -JUMPFORCE / 2 - GRAVITY * delta:
											if Global.power >= DOUBLEJUMP_COST:
												vel.y = JUMPFORCE
												jump_count += 1
...
func _on_Timer_timeout():
	...
	if Global.lives > 0:
		...
		SceneChanger.change_scene("res://Scenes/Menus/TitleScreen.tscn")


func drain_power(amount):
	Global._set_power(Global.power - amount)
	
func loss_lives(amount):
	Global._set_lives(Global.lives - amount)
...
func respawn():
	...
	set_collision_mask(8)
	invul_timer()
	if (Global.checkpoint_pos):
		set_global_position(Global.checkpoint_pos)
	else:
